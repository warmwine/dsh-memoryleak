...
- **收尾时把对话沉淀成笔记**。干完一段活，输入 `/ml note`：用当前模型把这一段对话压缩成工作记录、知识文件和结构化登记（详见下文「/ml note」专章）。整理过程与普通对话同款：思考块、markdown 摘要、写入工具卡片全程实时可见。花 token 的命令有 `/ml note`、`/ml ask` 和 `/ml mail read` 三个，其余命令全部本地完成。
...
**整理范围**：只处理「上一个 `/ml note` 之后 → 现在」的对话；本会话第一次执行则整理全部上下文。所以一支会话可以多次执行，每次只消化新的一段——不重复、不越压越胖。**只有成功完成的那次才算边界**：失败、取消或模型停摆被中止的 note 不消化它身前的对话，下次执行从更早的最后一次成功 note 起算，重试不丢内容。

**过程可见，与普通对话同款**：敲下回车立即出现开始信号（整理范围 + 模型 + 版本号）；模型的**思考过程**（reasoning）原样流式显示为可折叠思考块；模型输出**边流边解析**，以 markdown 分段实时显示——`**摘要**`、`**工作记录**`、`**知识条目**`、`**结构化登记 · 类别**` 依次出现，与普通对话同款的分块渲染（不再转发糊成一团的原始 JSON）。每次落盘写入是**真实的工具卡片**：写入进行中逐个出现并转圈，完成后翻到完成态（已创建 / 已合并 N 条 / 已写入 N 条工作记录）。随后气泡定格为一条「📌 已整理」的结构化回执（摘要 / 写入清单 / 注意事项），命令卡片另给一份纯文本回执（写入文件清单、token 用量）。失败时气泡定格为中断样式并给出错误，没写完的卡片如实显示为中断。工具卡片与真实回合的事件形状、顺序完全一致，不污染对话上下文；上一轮的写入记录也不会混进下一轮的整理区间。
...
- **过程与 /ml note 同款**：开始信号（引用文件数 + 模型）→ 思考块（若有）与回答以 markdown 逐字流式显示（与普通对话同款渲染）→ 完成后气泡定格并进入对话上下文（可以接着正常对话追问），命令卡片另给一份纯文本回执（含 token 用量）。
...
- **窗口**：只处理「上次 read 结束 → 现在」的新邮件，首次默认当天 00:00 起。结束时刻记在 Vault 根 `.memoryleak.yaml` 的 `mailState.lastReadEnd`（vault 限定键，GUI 保存不会冲掉，随 Vault 迁移）；**分析失败不推进进度**，重试覆盖同一窗口、不漏邮件。超单次上限保留最新、丢最旧的会如实告知。
- **两条铁律**：① 邮件原文只下载到**系统临时目录**（一次性目录，用完即删；崩溃残留由下次执行清扫 24 小时以上的旧目录），绝不写进 Vault 或当前工作区；② **下载与清理全程零模型调用**——窗口内没有新邮件时直接返回「没有新邮件」，连分析都不发生。
- **分析**（花 token，与 note/ask 同款流式显示）：当前模型通读后按严格 JSON 协议输出，代码侧白名单清洗，在会话里流式呈现**总评 / 重要事件 / 待办（需要处理，带期限）/ 待阅（值得一看）**，命令卡片回执含窗口、封数、token 用量与「临时目录已删除」说明。
