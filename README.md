...
- **收尾时把对话沉淀成笔记**。干完一段活，输入 `/ml note`：用当前模型把「上一个 `/ml note` 之后 → 现在」的对话（第一次执行则整理整个会话）压缩成三样东西——当天日志 `## NOTE` 段的工作记录、`MOMENTO/` 里的知识文件、`MOMENTO/databases.md` 等四张结构化登记表（数据库/服务器/凭证/术语，表格由代码渲染、按主键合并；凭证只记位置不记明文）。整理过程在会话里实时可见：开始信号立即出现，模型的压缩输出像普通回复一样逐字流式显示，完成后定格为一条结果摘要，最后命令卡片给出回执。这是唯一花 token 的命令，其余命令全部本地完成。
...
`/ml note` 的产出多一个 `MOMENTO/` 目录（知识库）和日志里的 `## NOTE` 段（工作记录）：

```markdown
## NOTE                     ← /ml note 压缩出的工作记录：每次整理一个 ### 小节，纯追加
...
- 表格合并只认声明的表头格式；**你手写的文件、格式对不上的内容永远不会被重写**——内容对不上时只会在文件末尾追加一个带日期的小节；
- 每次写入前有防丢失守卫（合并后主键必须包含合并前的全部，否则拒绝写入）；备份默认关闭（git 兜底），可在 vault 配置 `noteBackup: true` 开启（见下）。

模型只负责压缩与抽取（输出严格 JSON），文件格式与表格结构全部由插件代码决定：字段白名单、条数与长度上限、`|` 与换行清洗，模型输出无法破坏文件结构。
