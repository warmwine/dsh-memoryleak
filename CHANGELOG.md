...
- **todo 表单键盘快捷键**：类型+优先级组合卡支持数字键 `1/2/3` 选类型（deadline/sleep/anytime）、字母键选重要程度（`u`=urgent / `m`=medium / `l`=low，按选项首字母派生，chip 上有提示角标）；日期选择器支持数字键 `1-4` 直选今天/明天/本周/本月、`←→` 切月。Esc 取消照旧；焦点在输入框时不吞打字。
- 测试 364 → 407（新增 table-align、digest 流式摘要、ask 文法/纯逻辑/资料汇集/端到端用例）。
